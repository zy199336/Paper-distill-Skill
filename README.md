# Paper distill Skill Builder 文章蒸馏skill中文说明

本项目用于从一个小规模高质量论文 PDF 语料中，构建两个可复用的 Codex 写作技能：
...
```
安装后先重启一次 Codex，让 `paper-skill-builder` 被识别。
...
input_papers/
```
- PDF 数量应精，不宜多，选取和你想写文章最相关的，质量最高的文章，建议不超过30篇。

也可以使用绝对路径。如果提供 `input_papers`，脚本会在需要时递归扫描其中的 PDF。

### 3. 让 Codex 自动创建两个关键 skill

重启 Codex 并确认 `paper-skill-builder` 可用后，可以这样说：
...
- 如果没有提供论文目录，Codex 会先询问论文目录。
- Codex 运行 `python scripts/build_manifest.py --paper-dir "<paper-folder>"`。
- Codex 运行 `python scripts/create_extract_templates.py`。
- Codex 使用 `paper-corpus-analyzer` 工作流填写所有 included papers 的 `data/extracts/combined/PAPER_ID.md`。
- Codex 运行 `python scripts/validate_extracts.py`；如果抽取文件不完整，应补齐后重新校验。
- Codex 运行 `python scripts/synthesize_rules.py`。
- Codex 运行 `python scripts/sync_skills_from_synthesis.py`。
- Codex 运行 `python scripts/install_generated_skills.py`。
- Codex 运行 `python scripts/run_basic_checks.py --paper-dir "<paper-folder>"`。
- Codex 汇报 manifest、extracts、synthesis、已安装 skill 名称，并提醒重启。

成功安装后，Codex 应提示：

```text
已安装 writing-paper-outline 和 writing-paper。请重启 Codex，让新安装的两个 skill 被识别。
```

### 4. 重启 Codex

生成的两个 skill 安装完成后，需要重启 Codex。重启后可以直接调用：
...
- 也能够通过更改/data/paper_manifest.xlsx下的权重，自适应文章蒸馏强度
...
This project builds two reusable Codex writing skills from a focused set of high-quality academic paper PDFs:
...
After installation, restart Codex once so `paper-skill-builder` can be discovered.
...
### 3. Ask Codex To Create The Two Key Skills

After restarting Codex and confirming that `paper-skill-builder` is available, use a prompt like:
...
- If no paper folder is provided, Codex asks for the paper folder first.
- Codex runs `python scripts/build_manifest.py --paper-dir "<paper-folder>"`.
- Codex runs `python scripts/create_extract_templates.py`.
- Codex uses the `paper-corpus-analyzer` workflow to fill `data/extracts/combined/PAPER_ID.md` for all included papers.
- Codex runs `python scripts/validate_extracts.py`; if extraction files are incomplete, Codex should fix them and rerun validation.
- Codex runs `python scripts/synthesize_rules.py`.
- Codex runs `python scripts/sync_skills_from_synthesis.py`.
- Codex runs `python scripts/install_generated_skills.py`.
- Codex runs `python scripts/run_basic_checks.py --paper-dir "<paper-folder>"`.
- Codex reports the manifest, extracts, synthesis files, installed skill names, and restart reminder.

After successful installation, Codex should report:
...
After the two generated skills are installed, restart Codex. After restart, you can call:
